...
bgm.setAttribute("src", "./assets/Bgm tetris.mp3");
bgm.muted = true;

breakSound.setAttribute("src", "./assets/tetris pause.mp3");
bgm.muted = true;

drop.setAttribute("src", "./assets/drop or next.mp3");
drop.muted = true;
...
function canTetrominoMove(rowOffset, colOffeset) {
  for (let i = 0; i < currentTetromino.shape.length; i++) {
    for (let j = 0; j < currentTetromino.shape[i].length; j++) {
      if (currentTetromino.shape[i][j] !== 0) {
        let row = currentTetromino.row + i + rowOffset;
        let col = currentTetromino.col + j + colOffeset;
        if (
          row >= BOARD_HEIGHT ||
          col < 0 ||
          col >= BOARD_WIDTH ||
